...
import { locationOptions, experienceOptions, BasepayOptions, groupedOptions } from './data.ts';
...
function App() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.log(items)
      combinedFilter()
      //setFilteredItems(prevItems => [...prevItems, ...data.jdList]);
  ...
  const combinedFilter = () => {
    ...
    let newItem = items;
    console.log(items)
    // if (roleArray.length === 0 && experience == null && locationArray.length === 0 && basePay == null) {
    //   newItem = items;
    // }
    ...
    if (experience) {
      console.log("b")
      newItem = newItem.filter((newVal) => {
        return (experience.value >= newVal.minExp && experience.value <= newVal.maxExp)
      })
    }
    if (locationArray.length !== 0) {
      console.log("c")
      const selectedLabels = locationArray.map(item => item.label.toLowerCase());
      let tempItems = [];
    ...
    if (searchKeyword.length !== 0) {
      console.log("f")
      newItem = newItem.filter((newVal) => {
        return newVal.companyName.toLowerCase().includes(searchKeyword.toLowerCase())
      })
    }
    console.log(newItem)
      ...
      <div className='filters'>
        ...
        <Form>
          <Form.Group className="mb-3" controlId="formBasic">
            <Form.Control type="text" placeholder="Search Company Name" onChange={(e) => search(e.target.value)} />
          </Form.Group>
        </Form>
      </div>
            ...
            <Card className='cardBody'>
              <Card.Body>
                <div style={{ display: "flex", gap: "0.5rem" }}>
                  ...
                  <div>
                    <div>
                      ...
                      <h2 className='job-role'>{item.jobRole && `${item.jobRole.charAt(0).toUpperCase()}${item.jobRole.slice(1)}`}</h2>
                    </div>
                    <span style={{ fontSize: "11px" }}>{item.location && `${item.location.charAt(0).toUpperCase()}${item.location.slice(1)}`}</span>
                  </div>
                </div>
                <p className='estimated-salary'>Estimated Salary: {item.salaryCurrencyCode === 'USA' ? '$' : '₹'}  {item.minJdSalary} -  {item.salaryCurrencyCode === 'USA' ? '$' : '₹'} {item.maxJdSalary} LPA</p>
                ...
                <div className='view-job'>
                  <a href={item.jdLink} style={{ color: "#4943da", textDecoration: "none" }}>View Job</a>
                </div>
              ...
              <Button className='easy-apply-btn' size="lg">Easy Apply</Button>
            </Card>
          </Col>
        ))}
      </Row>
